---
title: "haplot walkthrough"
author: "Thomas Ng and Eric C. Anderson"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{haplot walkthrough}
...
microhaplot::mvShinyHaplot(tempdir()) # provide a directory path to host the haPLOType app
```  
...
The first thing you will want to get straight is that there are essentially three different "panels" on the haplot viewer that will always stay with you.  From top to bottom they are:
...
panel of amplicons. * - Note: the location and relative position of each button might change pending on the width of the viewing window.
    ...
    In the "+ read criteria" tab, you are given the choice to define the top $n$ alleles, minimal read depth and minimal allelic ratio that microhaplotypes must have to pass through. It is recommend to adjust your filter based on this particular order, starting with:
    * `Top n Alleles` defines the upper number of microhaplotypes (in a single individual's locus) to be kept starting from the highest read depth to lowest. Ideally, the top n alleles should match up to ploidy level of the species you study. So for a diploid organism, the *top n alleles* should be set to 2.   
      ...
      * `Minimal Allelic Ratio` defines the minimal allelic ratio that haplotype needs to pass. Allelic ratio is define as the ratio of the observed read depth of a microhaplotype to the read depth of the most common microhaplotype in an individual at a locus.
...
Haplot is equipped with three types of filtering criteria to clean out your microhaplotype. We will be describe what each of the criteria does and how is it best to define
* Top n Alleles: This keeps the first $n$ haplotypes observed in an individual's locus ordering from the highest read depth to lowest.d
