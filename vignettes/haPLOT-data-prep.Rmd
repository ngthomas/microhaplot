---
title: "An Introduction to haPLOType"
author: "Thomas Ng and Eric C. Anderson"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{haPLOT introduction}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

This vignette concerns itself with an overview of haPLOT and on 
preparation of data for input into haPLOT.  To learn about how to use
the graphical features of haPLOT, once your data have been read into haPLOT, see the 
`haPLOT walkthrough` vignette.

## How does haPLOT work?

haPLOT is a tool that makes it easy to extract, visualize, and interpret short-read haplotypes---or 
"microhaplotypes" in the parlance of some---from aligned sequences.
Before we go further, it will be good to step back and confirm what it is we are calling a short-read haplotype, or 
a "microhaplotype" within the context of haPLOT.  To define a microhaplotype
in haPLOT you need first to define a series of positions that a read might align to.  Then, haPLOT can analyze
the reads aligning at those positions, and extracts the sequence of the reads at those positions.  For example,
let's say that we know SNPs occur at positions 19, 27, 68, and 101 of a particular reference sequence that we have
aligned our reads to.  If a read aligns there and shows an `A` at position 19, a `G` at position 27, a `C` at position
68, and another `C` at position 101, then we say the microhaplotype on that read is `AGCC`.  haPLOT is a tool that
...
To understand what is involved in getting your data into haPLOT, it will be useful
to have a brief introduction to what haPLOT does and how it operates.  We will start 
by listing the things that haPLOT is **not**.

- haPLOT **is not a SNP-caller or variant detector**.  It is not the job of haPLOT to determine if a variant might exist in some location
or not.  You have to tell it the positions where it will find variants.  haPLOT just extracts the bases sequenced on
different reads at those positions.
- haPLOT **is not an alignment program**.  haPLOT does not do alignments.  You have to provide aligned data to it.  From the
aligned data, haPLOT can extract the bases sequenced at the desired locations. 
- haPLOT **is not a haplotype phasing program**. There is no special algorithm going on in haPLOT to try to assemble haplotypes (across
reads, for example.) It merely uses the simple fact that a single sequencing read will come from a single chromosome.  So, if you have 
can read the bases present at 4 variant positions along a 150-bp read, you know that those 4 bases constitute a haplotype.   
- haPLOT **is not a de-novo assembly algorithm**.  There is no assembly going on here at all!



## What kind of data can I use with haPLOT?
...
Though we use haPLOT for amplicon sequencing, it might also be useful for 
...
- We thought that [GATK's Unified Haplotype Caller](https://software.broadinstitute.org/gatk/gatkdocs/org_broadinstitute_gatk_tools_walkers_haplotypecaller_HaplotypeCaller.php)
...
sequencing data, GATK was brought to its kneees and didn't offer a viable solution. It appears that
...
The starting point for haPLOT requires:

1.  An indexed, BAM file of reads aligned to the reference sequences for each amplicon.
