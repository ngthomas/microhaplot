...
This vignette will walk you through a look at the example data and introduce you to many of the features of `microhaplot`.

This package is preloaded with two slim down haplotype data file from actual amplicon based sequence runs of two different marine species - `fish1.rds` and `fish2.rds`.
The data that we will be exploring will appear in the Shiny app dropdown menu
...
microhaplot::mvShinyHaplot(tempdir()) # provide a directory path to host the microhaplot app
```  
and then tells you to launch haplot like this:
```r
library(microhaplot)
...
Both `fish1.rds` and `fish2.rds` dataset are generated from a high throughput amplicon based sequence of one containing amplication from 22 locus points of 436 individuals of one population group, and one from 14 locus of 288 individuals from two populations, respectively. The actual run contains more loci but we subsample it in here to make the data protable. The motivation behind supplying two datasets is to show that, despite both are reduced genome representation amplicon reads, there is a wide range difference in sample and loci quality in between runs/species.  
...
user input panel, the panel with the light gray background.  
    ...
    <img src="field_selection.png" title="field selection tab" width="80%" height="70" />
    
    If the "> field selection" is selected, then the user input panel will include dropdown boxes that let you choose a group, locus, or individual(s) that you wish to focus on (this choice affects what is shown in different plots and outputs,
etc.) It also includes "backwards-and-forwards" buttons, "<" and ">", that sits immediate right to the selected field which allows you to roll backward or forward through the content of the selected field. 
...
panel of amplicons. * - Note: the location and relative position of each button might change pending on the width of the viewing window.
    ...
    In the "+ read criteria" tab, you are given the choice to define the top $n$ alleles, minimal read depth and minimal allelic ratio that microhaplotypes must have to pass through. It is recommend to adjust your filter with this particular order, starting from:
    * `Top n Alleles` defines the upper number of microhaplotypes (in a single individual's locus) to be kept starting from the highest read depth to lowest. Ideally, the top n alleles should match up to ploidy level of the species you study. So for a diploid organism, the *top n alleles* should be set to 2. 

      * `Min Total Read Depth` or _Minimal Total Read Depth_ defines the minimal total number of microhaplotype reads that a single individual's locus needs to pass. Importantly, we only add up the read depth of haplotypes that meets the `top n Alleles`. It can only take values greater than or equal to 0.
      * `Minimal Allelic Ratio` defines the minimal _allelic ratio_ that haplotype needs to pass. Allelic ratio or allelic balance is define as the ratio of the observed read depth of a microhaplotype to the read depth of the most common microhaplotype in an individual at a locus.

        > Ideally, for homozygous individuals, you expect to see the true microhaplotype to have a large read depth and an allelic ratio of one. False or noisy microhaplotypes will have low read depth and allelic ratio close to zero.

        > For a diploid individual that is truly heterozygous, the two true microhaplotypes ideally should have a large and similar number of reads, which means that their allelic ratio should be at or near one. The `Minimal Allelic Ratio` defines a filter that removes reads from haplotypes with allelic ratio less than the minimum.  It must be between 0 and 1.

        Once you set up these three criteria, you _apply_ them with the "apply" button. You can also save these criteria if you want to visit them in the future by clicking the "save" button.There are other options that located on the right hand side that gives you more ways to define filtering criteria. Details for each of the options are shown if you hover your pointer over it.

    c. **locus annotation:**
     
    <img src="locus_annotation.png" title="locus annotation tab" width="80%" height="70" />
...
1. **The main body panel:** This occupies most of microhaplot's screen real estate.  It is where the actual
...
of the main body panel:
  <img src="main_tab.png" title="main tab" width="60%" height="40" />
    a. *Panel:* the place where you choose which data set you want to explore.  When you started microhaplot it should have had the dropdown choice for "fish1.rds".

    b. *Summary Info: * displays condensed graphical summary of the characteristics of microhaplotype reads by groups, individuals, or loci.
    ...
    f. *About:* provides a little background about microhaplot and how to cite it and whom to contact about it.
