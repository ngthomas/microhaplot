...
panel of amplicons. * - Note: the location and relative position of each button might change pending on the width of the viewing window.
   ...
   The _allelic ratio_ or _allelic balance_ is defined as the ratio of the observed read depth of a microhaplotype to the read depth of the most common microhaplotype in an individual at a locus.
   ...
   For an individual that is truly heterozygous, the two true microhaplotypes ideally should have a large and similar number of reads, which means that their allelic ratio should be at or near one. The _Minimal Allelic Ratio_ defines a filter that 
   removes reads from haplotypes with allelic ratio less than the minimum.  It must be between 0 and 1.
...
represented (the reason for this is that if you have many loci, you don't want to end up waiting fo an
...
histogram-like information.  The y-axis has different loci on different lines.  The x-axis is the number of unique haplotypes
...
At this point, we may as well have a tour of the facilities for visualizing the data
...
depths at this individual s288.  Where it says "Show 15 entries" you should change the "15"
